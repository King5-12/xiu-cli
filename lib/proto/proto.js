...
const loadProto = (pathList) => {
    ...
    return new Promise((res) =>
        root.load(pathList, option, (err, root) => {
            res({ err, root });
        }),
    );
};
...
const buildMethods = (node) => {
    ...
    pushMethod(
        ...
        isTs
            ...
            : `${node.name} : async (${requestTypeName}) => {`,
    );
    ++indent;
    pushMethod(
        node.name,
...
const runProto = async (pathList, type) => {
    //设定全局语言
    isTs = type === 'ts';

    const { err, root } = await loadProto(pathList);
